          ...
          FWHM = 0.09, SBlim = 26, df = 100, raw = FALSE, plot = TRUE, main_src = NULL, GRAFitlib = GRAFitlib) 
{
  ...
  if (plot) {
    ...
    lines(predict.image, col = "black", lwd = 1.5)
    lines(predict.bulge, col = "red", lwd = 1.5)
    lines(predict.disk, col = "blue", lwd = 1.5)
    lines(predict.total, col = "darkgreen", lwd = 1.5)
    lines(psfellipse, col = "purple", lwd = 1.5)
    abline(h = SBlim, lty = 3, lwd = 1.2)
    abline(v = xhi, lty = 3, lwd = 1.2)
    abline(v = FWHM/2, lty = 3, lwd = 1.2)
    abline(v = modellist$sersic$re[1]*pixscale, col = "red", lty = 3, lwd = 1.2)
    abline(v = modellist$sersic$re[2]*pixscale, col = "blue", lty = 3, lwd = 1.2)
    legend("topright", legend = c("Data", "ProFit Total", "ProFit Bulge", 
                                  "ProFit Disk", "PSF"), lty = 1, 
           col = c("black", "darkgreen", "red", "blue", "purple"), bg = "white")
    magplot(predict.image$x, predict.image$y - predict.total$y, 
            type = "l", xlim = c(0, xhi), ylim = c(-0.5, 0.5), col = "darkgreen", 
            xlab = xlab, ylab = ylab2, grid = FALSE, labels = c(T, T), lwd = 1.5)
